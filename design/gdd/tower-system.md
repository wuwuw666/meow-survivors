...
- `apply_tower_mod(tower_node: Node, mod_id: String) -> void`
- `get_runtime_tower_stats(tower_type: String) -> Dictionary`
...
在当前 `C-Tangy` 成长结构中，塔的成长不再来自 XP 升级面板，也不再通过“点击塔直接花金币升级”的局内流程完成。
...
- 读取 `tower-meta-progression-system` 提供的基础加成
- 读取 `tower-mod-system` 提供的单局改造结果
- 在塔创建时把基础成长应用到实例
- 在战斗中允许对已存在塔追加改造行为
...
```text
get_runtime_tower_stats(tower_type):
    读取塔基础数据
    应用局外基础强化
    返回本局应使用的运行时属性

apply_tower_mod(tower_node, mod_id):
    验证改造件兼容性
    将改造行为绑定到塔实例
    刷新该塔的运行时表现
```
...
- 塔的基础强度可被局外成长轻量推动
- 塔的本局花样优先来自改造件，而不是 XP 面板
- 每次成长都应回答“这座塔更稳了”或“这座塔打法变了”
...
| **金币系统** | 硬依赖 | 检查/扣除金币 | 放塔消耗与经济约束 |
| **地图系统** | 硬依赖 | 提供塔位位置与占用状态 | 塔位放置基础 |
| **解锁系统** | 软依赖 | `is_tower_unlocked(tower_type)` | 判定某塔是否对当前玩家可用 |
...
| **塔改造系统** | 硬依赖 | `apply_tower_mod(...)` | 为具体塔实例追加本局行为变化 |
| **塔局外成长系统** | 软依赖 | `get_runtime_tower_stats(...)` | 提供塔基础强化与解锁状态 |
...
| AC-TW-10 | 塔运行时属性接入 | 创建塔实例 | 已解锁的基础强化能被正确读取并应用 |
| AC-TW-11 | 塔改造接口可调用 | 调用 `apply_tower_mod(...)` | 该塔行为按改造定义发生明显变化 |
...
| AC-TW-I05 | 塔 + 局外成长系统 | 局外基础强化能真实改变塔基础表现 |
| AC-TW-I06 | 塔 + 改造系统 | 改造能真实改变塔的行为或联动方式 |
| AC-TW-I07 | 塔 + UI 系统 | 放塔、命中、buff 变化都有可见反馈 |
