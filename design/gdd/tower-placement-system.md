...
当前 `C-Tangy` 结构下，点击已占塔位**不再触发局内花金币升级**。
...
- 塔的局内变化来自改造件，而不是独立升级按钮
- 塔的长期成长来自局外成长节点，而不是战斗中额外金币漏斗
- 塔位交互应优先帮助玩家理解“这座塔现在在做什么”
...
| 系统 | 依赖类型 | 接口 | 说明 |
|------|---------|------|------|
| **防御塔系统** | 硬依赖 | `place_tower(type, pos)`, `get_tower_data(type)` | 创建塔实例并查询塔数据 |
| **金币系统** | 硬依赖 | `player_coins`, `deduct_coins()`, `refund_coins()` | 放置费用管理 |
| **地图系统** | 硬依赖 | `get_tower_slots() -> Array[TowerSlotData]` | 获取所有塔位定义 |
| **解锁系统** | 软依赖 | `get_unlocked_towers()` | 过滤当前可建造塔列表 |
...
| AC-TP-10 | 查看已有塔信息 | 点击 OCCUPIED slot | 弹出塔信息面板，显示类型、作用与关键属性 |
| AC-TP-11 | 已占塔位不进入升级链路 | 点击 OCCUPIED slot | 不出现局内金币升级按钮 |
| AC-TP-12 | tower_placed 信号 | 监听信号 | 每次成功放置恰好 1 次，参数(slot_id, tower_type) 正确 |
| AC-TP-13 | 解锁过滤生效 | 打开放置面板 | 未解锁塔不会出现在可建造候选里 |
